...
import { authenticateAccount, closeAuthDb, ensureAuthSchema, listRegisteredAccounts, registerAccount, updateRegisteredAccountStatus } from "./authDb.js";
import {
  authenticateStudent,
...
app.post("/api/admin/users/list", async (req, res) => {
  ...
  try {
    registeredAccounts = await listRegisteredAccounts();
  } catch (error) {
    console.error("Registered account list error:", error.message);
  }
  const users = [...listStudents(), ...registeredAccounts]
    .filter((user, index, list) => list.findIndex((item) => String(item.email || item.phone || item.id) === String(user.email || user.phone || user.id)) === index)
    .map((student) => sanitizeStudent(student));
  return res.json({
    ok: true,
...
app.post("/api/meetings", async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      console.error("Meeting notification account lookup error:", error.message);
    }
    const participants = [...listStudents(), ...registeredAccounts]
      .filter((user, index, list) => list.findIndex((item) => `${item.role}:${item.id}` === `${user.role}:${user.id}`) === index)
      .filter((user) => meeting.participant_ids.includes(Number(user.id)));
...
app.post("/api/admin/users/:phone/approve", async (req, res) => {
  ...
  const updated = updateStudentStatus(targetPhone, "approved", actor.full_name) || await updateRegisteredAccountStatus(targetPhone, "approved", actor.full_name);
  if (!updated) {
    return res.status(404).json({ ok: false, message: "User not found." });
  }

  const deliveries = await sendApprovalStatusNotification({ student: updated, status: "approved", reviewedBy: actor.full_name });
  return res.json({
    ok: true,
...
app.post("/api/admin/users/:phone/deny", async (req, res) => {
  const actor = requirePrivilegedActor(req, res);
  if (!actor) {
    return;
  }
  ...
  const updated = updateStudentStatus(targetPhone, "denied", actor.full_name) || await updateRegisteredAccountStatus(targetPhone, "denied", actor.full_name);
  if (!updated) {
    return res.status(404).json({ ok: false, message: "User not found." });
  }

  const deliveries = await sendApprovalStatusNotification({ student: updated, status: "denied", reviewedBy: actor.full_name });
  return res.json({
    ok: true,
